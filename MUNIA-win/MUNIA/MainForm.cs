...
namespace MUNIA {
    public partial class MainForm : Form {
        ...
        public SvgController Controller { get; set; }
		...
		private void BuildMenu() {
			...
			foreach (string svgPath in Directory.GetFiles("./skins", "*.svg")) {
				...
				nocontroller:
					...
					tsmi.Click += delegate {
						Controller = svgc;
						Controller?.Render(glControl.Width, glControl.Height);
	    ...
	    protected override void WndProc(ref Message message) {
		    ...
		    case Const.WM_INPUT:
			    ...
			    Controller?.WndProc(ref message);
			    break;
		    }
	    ...
	    private void DownloadAndUpdate(string url) {
		    ...
		    wc.DownloadProgressChanged += (sender, args) => BeginInvoke((Action)delegate { UpdateStatus(string.Format("downloading, {0}%", args.ProgressPercentage * 95 / 100), args.ProgressPercentage * 95 / 100); });
